...
RENDER_VARS='${IMAGE_TAG} ${KEYCLOAK_ADMIN} ${KEYCLOAK_CLIENT_SECRET} ${KEYCLOAK_HOST} ${KEYCLOAK_REALM} ${KEYCLOAK_TLS_SECRET_NAME} ${LETSENCRYPT_EMAIL} ${MYVAULT_HOST} ${NAMESPACE} ${OPENAI_EMBEDDING_MODEL} ${OPENWEBUI_HOST} ${OPENWEBUI_IMAGE} ${PIPELINES_IMAGE} ${REGISTRY} ${SCW_LLM_BASE_URL} ${SCW_LLM_MODEL} ${SEARXNG_HOST} ${SEARXNG_IMAGE} ${SEARXNG_TLS_SECRET_NAME} ${TLS_SECRET_NAME} ${VALKEY_IMAGE}'
...
: "${MYVAULT_HOST:=myvault.fake-domain.name}"
export BRIDGE_HOST CORPUS_MANAGER_HOST MYVAULT_HOST
